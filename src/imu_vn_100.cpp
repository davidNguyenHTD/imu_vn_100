...
void FillImuMessage(sensor_msgs::Imu& imu_msg, vn::protocol::uart::Packet& p,
                    bool binary_output);
void FillImuMessage(sensor_msgs::Imu& imu_msg, vn::protocol::uart::Packet& p,
                    bool binary_output, bool use_right_hand_rule);
...
void ImuVn100::LoadParameters() {
  ...
  pnh_.param("enable_rpy", enable_rpy_, true);
...
void ImuVn100::CreateDiagnosedPublishers() {
  ...
  if (enable_mag_) {
    pd_mag_.Create < sensor_msgs::MagneticField
        > (pnh_, "magnetic_field", updater_, imu_rate_double_);
  }
  if (enable_pres_) {
    pd_pres_.Create < sensor_msgs::FluidPressure
        > (pnh_, "fluid_pressure", updater_, imu_rate_double_);
  }
  if (enable_temp_) {
    pd_temp_.Create < sensor_msgs::Temperature
...
void errorMessageReceived(void* userData, vn::protocol::uart::Packet& p,
  ...
  switch (se) {
    ...
    case SensorError::ERR_WATCHDOG_RESET:			///< Watchdog reset
      ROS_WARN(
          "VN: Watchdog reset has occured. VN should have restarted within 50 ms.");
      break;
    ...
    case SensorError::ERR_INSUFFICIENT_BAUD_RATE:  ///< Insufficient baud rate.
      ROS_WARN(
          "VN: Insufficient baud rate for requested async data output and rate.");
      break;
